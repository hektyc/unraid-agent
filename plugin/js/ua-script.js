...
(function($) {
  ...
  // Generic sub-tab activation: clicks on .ua-subtab toggle the matching
  // .ua-pane or .ac-pane content panel. Works for both pages because we
  // query both pane classes.
  $(document).on('click', '.ua-subtab[data-uatab]', function() {
    var t = $(this).data('uatab');
    if (!t) return;

    // Distinguish top-level tabs from tool-group tabs (tg-*)
    var isToolGroup = (t.indexOf('tg-') === 0);

    if (isToolGroup) {
      // Tool group sub-tabs: only toggle within the tg- scope
      ...
      // Top-level sub-tabs: toggle both top-level and tool-group tabs
      ...
      // Persist the active tab per page-context
      // Use a heuristic: if the page has tool domains, it's the Permissions page
      if ($('.ua-domain').length) {
        localStorage.setItem(PERMS_KEY, t);
        if (t === 'tools') {
          // Auto-activate saved tool group when switching to Tools tab
  ...
  // On page load, restore the last active sub-tab or default to first
  function restoreActiveTab() {
    var $panes = $('.ua-pane[data-uatab], .ac-pane[data-uatab]');
    if ($panes.length === 0) return;

    var $activePane = $('.ua-pane.active, .ac-pane.active');
    if ($activePane.length) return; // already has an active pane
    ...
    if (!$target || !$target.length) {
      // Default to first non-tool-group tab
  ...
  // Expand first domain by default (so the user sees something useful)
  ...
  $(document).on('click', '.ua-card[data-entity]', function(e) {
    // Skip if clicking the export chip
  ...
  function openContentModal(kind, title, content) {
    ...
    // Extract description from frontmatter if editing existing skill
  ...
  $(document).on('click', '#ac-modal').on('click', function(e) {
    if (e.target === this) { $('#ac-modal').fadeOut(120); currentModal = null; }
  });
  $(document).on('click', '#ua-modal').on('click', function(e) {
